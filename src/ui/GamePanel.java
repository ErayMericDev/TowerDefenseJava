...
private int enemiesInWave = 5;        // Bu dalgadaki düşman sayısı
private int enemiesSpawned = 0;       // Bu dalgada spawn olan
private int enemiesKilledInWave = 0;  // Bu dalgada ölen
private int spawnCounter = 0;
private int spawnInterval = 150;      // Spawn aralığı (frame)
...
private boolean waveInProgress = false;
    ...
    for (int i = 0; i < types.length; i++) {
        ...
        // Fiyat label'ı ekle (butonun altında)
...
add(sellBtn);
// SELL label'ı ekle (butonun altında)
...
add(sellLabel);
   ...
   leaveBtn.addActionListener(e -> {
    ...
    // Spawn aralığını azalt (daha hızlı spawn)
    ...
    // Wave'e göre güçlenen düşmanlar
    double waveMultiplier = 1 + (currentWave - 1) * 0.2;  // Her wave %20 artış
    
    int enemyHealth = (int) (baseEnemyHealth * waveMultiplier);
    double enemySpeed = baseEnemySpeed + (currentWave - 1) * 0.1;  // Her wave +0.1 hız
    
    // Her 5 düşmandan 1'i tank (wave 2'den sonra)
    ...
    protected void paintComponent(Graphics g) {
        ...
        // 7. Kule önizleme
        if (selectedTowerType != null && mouseY > BAR_HEIGHT) {
            drawTowerPreview(g2d);
        }
        
        // 8. Seçili kuleyi vurgula (SELL için)
        if (selectedTower != null) {
